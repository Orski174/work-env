...
for i in $(seq 1 30); do
  out="$(ssh_to "$CFG" "${ALIAS[pg1]}" 'cd ~/patroni && timeout 15 sudo docker compose exec -T patroni patronictl -c /etc/patroni.yml list' 2>/dev/null || true)"
...
ssh_to "$CFG" "${ALIAS[pg1]}" 'cd ~/patroni && timeout 15 sudo docker compose exec -T patroni patronictl -c /etc/patroni.yml list' 2>&1
